...
class GenerateCode(ast.NodeVisitor):
            ...
            if isinstance(value, list):
              for item in value:
                if isinstance(item, ast.AST):
                  self.visit(item)
                if(self.InFunc):
                  x=self.new_temp("param")
                  self.code.append(("Build_Param", item.split(" ")[1], x))
                  self.code.append(("Set_ParamType", x, item.split(" ")[0]))
                  self.code.append(("Set_FuncParam", self.functName, x))
        ...
        def visit_FuncDefn(self, node):
          x=self.new_temp("function")
          self.functName=x
          self.code.append(("Build_Function", x))
          self.InFunc=True
          for field in getattr(node, "_fields"):
        ...
        def visit_TypeNode(self, node):
          for field in getattr(node, "_fields"):
            ...
            x=self.new_temp("type")
            self.code.append(("Build_Type", value, x))
            self.code.append(("Set_LocalVarType", x, value))


        def visit_Assign(self, node):
          self.optemp=[]   #Lista temporal de las operaciones a realizar
          for field in getattr(node, "_fields"):
            ...
            if(self.InFunc):
              #print(value) #valorID
              #print(field) #"ID"
              x=self.new_temp("assign")
              self.code.append(("Build_AssignFunc", value, x))
              self.code.append(("Set_ParamAssignFunc", x, value))
              self.InFunc=False
            else:
              x=self.new_temp("assign")
              self.code.append(("Build_Assign", value, x))
              self.code.append(("Set_ParamAssign", x, value))
